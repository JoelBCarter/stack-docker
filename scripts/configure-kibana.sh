...
until curl -s http://127.0.0.1:5601/login -o /dev/null; do
...
curl -s -XPUT http://127.0.0.1:changeme@elasticsearch:9200/.kibana/index-pattern/logstash-* \
     -d '{"title" : "logstash-*",  "timeFieldName": "@timestamp"}'

# Set the default index pattern.
curl -s -XPUT http://127.0.0.1:changeme@elasticsearch:9200/.kibana/config/${ELASTIC_VERSION} \
